...
class GreedyGoto(ss.Tactic):
    ...
    def init(self, params):
        ...
        self._flag = int(params['First Iteration'])
    ...
    def getcounts():
        with open("radfile.csv", 'r') as radfile:
            firstline = radfile.readline()
            counts = int(firstline)
            return(counts)
    ...
    def step_autonomy(self, t, dt):
        ...
        if self._time_at_wp > TIME_AT_WP:
            ...
            try:
                ...
                global flagstring
                # messageArray = [self._id, self._wp_id_list, self._loc, self._wp_id_list.index(self._loc), self._flag]

                ################################ HEATMAP ################################

                lat = float(usma_enums.WP_LOC[self._loc][0])
                lon = float(usma_enums.WP_LOC[self._loc][1])
                alt = 1 # TODO
                counts = getcounts()
                messageArray2 = "{0} {1} {2} {3}".format(lat, lon, counts, alt)
                print("Sending Message to Base Station...")
                sock.sendall(messageArray2)

                #########################################################################

                #message = str(self._id) + "Working WP List:" + str(self._wp_id_list) + "\nFinished WP:" + str(self._loc) + "\nAt Index:" + str(self._wp_id_list.index(self._loc)) + str(flagstring)
