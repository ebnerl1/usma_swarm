...
class GreedyGoto(ss.Tactic):
    ...
    def init(self, params):
        ...
        self._id_in_subswarm.append(131) #Getrid of error message

    def callbackSetup(self):#BB
        self._parent.createSubscriber('/altitude', PointStamped, self.chatter_cb)

    def chatter_cb(self, msg):#BB
        self._parent.log_info("The altitude is: %f" %msg.point.x)

    def receive_altitude(self, msg):
        print >>sys.stderr, "altitude received: %s" % msg.x

        with open(self.radeyeDir + 'deletme.csv', 'w+') as f:
            firstline = f.writeline(msg.x)
        self.altitude=msg
