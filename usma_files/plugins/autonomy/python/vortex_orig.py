...
class Vortex(ss.Tactic):
    def init(self, params):
        ...
        self._bboxes = gps.get_battleboxes()
    ...
    def step_autonomy(self, t, dt):
        ...
        if self._bboxes[0].contains(own_lat, own_lon, own_alt):
            if (self._team is 'blue' and self._bboxes[2].contains(own_lat, own_lon, own_alt)) or \
                    (self._team is 'red' and self._bboxes[1].contains(own_lat, own_lon, own_alt)):
                self._weapons_range = enums.MAX_RANGE_DEF
            else:
                self._weapons_range = enums.MAX_RANGE_OFF
