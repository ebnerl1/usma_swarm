'''
Tactic Name: Greedy Shooter Climb To Altitude v1 (greedy_shooter_climb_to_altitude_v1.py)
Description:  This tactic is a variation of the GreedyShooterSingle in greedy_shooter_single.py
...
class GreedyShooterSingle(ss.Tactic):
    ...
    def step_autonomy(self, t, dt):
            ...
            for uav in self._reds.values():
                if uav.vehicle_id not in self._shot:
                    ...
                    if d < min_dist:
                        ...
                        if self._target_id != self._tracked:
                            self._tgt_find_count = 0
        ...
        if self._target_id != None:
            ...
            self._wp = np.array([lat, lon, attack_alt ])  # np.array([lat, lon, target_pos.rel_alt])
