...
# needs to be sourced, it also may need to be built first 
# using catkin_make.
...
class ObjectDetResponse(ss.Tactic):
    ...
    def step_autonomy_obj1_detected(self, t, dt):
        ...
        self._desired_lat = float(usma_enums.WP_LOC[self._loc][0]) + random.uniform(0.0003,0.0004)
        self._desired_lon = float(usma_enums.WP_LOC[self._loc][1]) + random.uniform(0.0003,0.0004)
        self._at_wp = False
