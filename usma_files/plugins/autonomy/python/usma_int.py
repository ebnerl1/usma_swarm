...
class USMAInt(ss.Tactic):
    ...
    def init(self, params):
        self._own_uav_id = int(params['id'])
        ...
        self._VT_turn_type = PNInterceptor.DIRECT_TURN
        self._s_turn_state = PNInterceptor.S1
        self._psi_p_f = 0.0
        self._gain = 0.0
        self._bias = 0.0
        self._E = 1e9
...
#        return self.is_active
    ...
    def step_autonomy(self, t, dt):
        ...
        if self._state == PNInterceptor.RUN_IN:
            self._runin_state()
    ...
    def _offset_state(self):
        ...
        if not self._state_initialized:
            ...
            if angle_between(psi_p_f_rel, 0.0, math.pi):
                self._VT_turn_type = PNInterceptor.S_TURN
                self._s_turn_state = PNInterceptor.S1
            elif angle_between(psi_p_f_rel, (self._theta_VT_init - \
                                             self._psi_p_VT_init), 0.0):
                self._VT_turn_type = PNInterceptor.DIRECT_TURN
            else:
                self._VT_turn_type = PNInterceptor.DIRECT_2BIAS
            self._state_initialized = True
        ...
        if self._VT_turn_type == PNInterceptor.DIRECT_TURN:
        ...
        elif self._VT_turn_type == PNInterceptor.DIRECT_2BIAS:
...
#                              self._theta2, self._theta3))
    ...
    def _parallel_2_state(self):
        ...
        '''
        if not self._state_initialized:
            self._psi_p_f = gps.normalize(self._xt[2] - PNInterceptor.FIRE_ANGLE)
            self._bias = 0.0
            self._state_initialized = True
        self._set_ctrl_params(self._xp, self._vp, self._xt, self._vt)
