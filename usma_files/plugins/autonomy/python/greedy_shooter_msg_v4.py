'''
Tactic Name: Greedy Shooter Messaging version 4 (GreedyShooterMsg4)
Description:  This tactic is a variation of the TutorialGreedyShooter in tutorial_greedy_shooter_1.py
              ...
              self._target_score.  It is a variation of GreedyShooterMsg3 in that there is a compound
...
Date:		6 Apr 2017
Changes:	1.  (6 Apr 2017): Changed alt param to keep uav from changing to target alt
            2.  (7 Apr 2017): Inital sort on distance, then auction on bearing
...
class GreedyShooterMsg4(ss.Tactic):
    ...
    def receive_msg_stat(self, msg):
        ...
        if msg.id == self._target_id:
            if msg.count < int(self._target_dist) and msg.latency < self._fov_width:
                l = len(self._target_list)
                if l > 1:
                    self._target_list.pop()
                    l = len(self._target_list)
                    (self._target_id, self._target_dist, self._target_bearing) = self._target_list[l-1]
                    #print '{} dropped UAV {} due to another blue being closer at distance {} and bearing {}'.format(self._id, msg.id, msg.count, msg.latency)
                    print 'Change: {} now targeting UAV {} at distance {} bearing {}'.format(self._id, self._target_id, self._target_dist, self._target_bearing)
