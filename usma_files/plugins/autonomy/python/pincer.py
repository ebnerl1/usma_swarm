...
class Pincer(ss.Tactic):
    ...
    def step_autonomy(self, t, dt):
        ...
        if self._state == States.new_start:
            ...
            if gps.gps_distance(own_lat, own_lon, self.ordered_corners[0][0], self.ordered_corners[0][1]) < 50:
                ...
                self.check_for_sideline()

        if self._state == States.patrol:
        ...
        if self._state == States.patrol_sideline:
            lat, lon = self.fixed_wing_waypoint(own_lat, own_lon, self.ordered_corners[1][0],
                                                self.ordered_corners[1][1])
            if gps.gps_distance(own_lat, own_lon, self.ordered_corners[1][0], self.ordered_corners[1][1]) < 25:
                self.ordered_corners = self.closestCorners()
        ...
        right_lat, right_lon = gps.gps_newpos(own_lat, own_lon, gps.normalize(own_bearing+(math.pi/2)), 50) #Planes take right turns
    ...
    def check_for_sideline(self):
        if self._bboxes[2].geography_contains(self.ordered_corners[0][0],self.ordered_corners[0][1]) and \
                self._bboxes[4].geography_contains(self.ordered_corners[1][0],self.ordered_corners[1][1]) or \
                self._bboxes[4].geography_contains(self.ordered_corners[0][0], self.ordered_corners[0][1]) and \
                self._bboxes[2].geography_contains(self.ordered_corners[1][0], self.ordered_corners[1][1]):
            print("Patrolling Sideline!")
            self._state = States.patrol_sideline
