...
void steerToWall2(int wallFind){
  if(wallFind == 0){
    left90();
  }
  else{
  ...
  wait(1000);
    /*while the car is greater than 5 inches away from the wall it is steering to...*/
  
  /*DRIVE FORWARD UNTIL IT GETS WITHIN 1 FOOT OF WALL*/
  driveForward2((Finches/10)*250);
...
void findLight(){
  ...
  //Serial.println("Left Sensor: " + String(sensorValue0/20));
  //.println("Front Sensor: " + String(sensorValue1/20));
  //Serial.println("Right Sensor: " + String(sensorValue2/20));
  //Serial.println();
  //delay(200);
  //if(orientation < 0){
  ...
  //  while(orientation > 45 || orientation < 315){
  //    right45();
  //  }
  //  driveForward2(2000);
  //}
  Serial.println(sensorValue1);
  while(sensorValue1 > 1.5 && sensorValue0 > 1.5 && sensorValue2 > 1.5){
    fTraverse();
    Serial.println(sensorValue1);
  }
  wait(2000);
  if (sensorValue0 > sensorValue1 && sensorValue0 > sensorValue2) {
  ...
  else if (sensorValue2 > sensorValue0 && sensorValue2 > sensorValue1) {
